...
class PerformanceMonitor:
    ...
    def record_error(self, error: str):
        self.stats["errors"].append({"timestamp": datetime.now(), "error": error})
    ...
    def get_summary(self) -> dict:
        ...
        avg_visit_time = (
            sum(self.visit_times) / len(self.visit_times) if self.visit_times else 0
        )
        ...
        return {
            ...
            * 100,
            "visit_success_rate": (
                self.stats["successful_visits"] / max(self.stats["total_visits"], 1)
            )
            * 100,
            ...
            "proxy_failure_rate": (
                self.stats["proxy_failures"] / max(self.stats["total_searches"], 1)
            )
            * 100,
            "captcha_rate": (
                self.stats["captcha_encounters"] / max(self.stats["total_searches"], 1)
            )
            * 100,
        }
