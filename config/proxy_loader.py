...
def filter_proxies_by_criteria(
    ...
    for proxy in proxies:
        if (
            proxy.latency <= max_latency
            and proxy.success_rate >= min_success_rate
            and proxy.is_active
        ):
...
def create_proxy_from_csv_row(row: Dict[str, str]) -> Optional[ProxyConfig]:
    try:
        ...
        port_int = int(port)
        latency_int = (
            int(latency_str.replace(" ms", ""))
            if " ms" in latency_str
            else int(latency_str)
        )
...
def load_proxies_from_csv_advanced(
    csv_file: str = "proxies-export.csv",
    max_proxies: int = 0,
    min_latency: int = 0,
    max_latency: int = 500,
